...
def solution(n, stations, w):
    ...
    for s in stations:
        ...
        between = end - start
        if between > 0:
            add = (between + w_range - 0.5) // w_range
            answer += int(add)

        start = s + w + 1
        start = start if start < n + 1 else n + 1
    # last
    between = n + 1 - start
    if between > 0:
        add = (between + w_range - 0.5) // w_range
...
if __name__ == '__main__':
    ...
    # all
    _stations = [2, 5, 6, 8, 10]
    print(solution(_n, _stations, _w))  # return 0

    # first
    _stations = [2]
    print(solution(_n, _stations, _w))  # return 3
    ...
    # normal
    _stations = [3, 9]
    print(solution(_n, _stations, _w))  # return 2

    _n = 11
    _stations = [4, 11]
    _w = 1
    _result = 3
    print(solution(_n, _stations, _w))
