...
# 다음과 같은 것들을 정의합니다.
#
# 어떤 수열 x의 부분 수열(Subsequence, https://en.wikipedia.org/wiki/Subsequence)이란,
...
def solution(a):
    numbers = {}
    for aa in a:
        if aa not in numbers:
            numbers[aa] = 0
        numbers[aa] += 1
    keys = sorted(list(numbers.keys()), key=lambda x: numbers[x], reverse=True)
    star_sequence = []
    for k in keys:
        if len(star_sequence) >= numbers[k] * 2:
            # maximum length with number k is two times of count of k.
            break
        sequence = get_star_sequence(a, k)
        if len(star_sequence) < len(sequence):
            star_sequence = sequence

    return len(star_sequence)
