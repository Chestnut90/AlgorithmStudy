...
# (연산의 두 대상 사이에 연산기호를 사용하는 방식)으로 표현된 연산식입니다. 잘못된 연산식은 입력으로 주어지지 않습니다.
...
def calculator(expression, orders):
    calc = {'*': lambda x, y: x * y,
            '+': lambda x, y: x + y,
            '-': lambda x, y: x - y}
    operands = []
    ...
    for ex in expression:
        ...
        a = operands.pop()
        b = operands.pop()
        operands.append(calc[ex](b, a))
    return operands[-1]

def to_postorder(expression, orders):
    _expression = []
    operators = []
    for exp in expression:
        if exp not in orders:
            _expression.append(int(exp))
            continue
        if operators:
            while operators:
                if orders[exp] <= orders[operators[-1]]:
                    _expression.append(operators.pop())
                    continue
                break
...
def solution_(expression):
    operators = ['*', '+', '-']
    sub_expression = re.sub(r'([*|+|-])', r' \1 ', expression).split(' ')
    answer = 0
    for i in permutations(operators, 3):
        oprs = {i[0]:2, i[1]:1, i[2]:0}
        postfix = to_postorder(sub_expression, oprs)
        r = calculator(postfix, oprs)
        if abs(r) > answer:
            answer = abs(r)
    return answer
...
'''
다른 풀이.!
괄호가 없어서 operator의 priority만 가지고 연산가능.
우선순위 연산자의 앞뒤 계산 후 리스트 업데이트.
'''
...
if __name__ == '__main__':
    ...
    _expression = "100-200*300-500+20"
    _result = 60420
    subs = re.sub(r'([*|+|-])',r' \1 ', _expression)

    r = solution(_expression)
    print(r)
